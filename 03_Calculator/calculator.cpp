...
#include <QLineEdit>
namespace {
QString stripEnclosingParentheses(QString value)
{
    value = value.trimmed();
...
{
    QString cleaned = stripEnclosingParentheses(text);
    cleaned.remove(' ');
    ...
    if (!cleaned.contains('j')) {
        return std::complex<double>(cleaned.toDouble(), 0.0);
    }
    ...
    if (splitIndex == -1) {
        ...
        return std::complex<double>(0.0, imagPartText.toDouble());
    }

    const QString realPartText = stripEnclosingParentheses(withoutJ.left(splitIndex));
    const double realPart = realPartText.toDouble();
    QString imagPartText = stripEnclosingParentheses(withoutJ.mid(splitIndex));
    if (imagPartText == "+") {
        imagPartText = "1";
    } else if (imagPartText == "-") {
        imagPartText = "-1";
    ...
    QWidget(parent), ui(new Ui::Calculator)
{
    ui->setupUi(this);
    oper = None;
    numA = std::complex<double>(0.0, 0.0);
    numB = std::complex<double>(0.0, 0.0);
}
